...
ifeq ($(WIN32), 1)
DLLNAME=darknet
WINOPTS=-fvisibility=hidden -static-libgcc -static-libstdc++ -Wl,--output-def,$(DLLNAME).def
BUILDDLL=lib /machine:i386 /def:$(DLLNAME).def
WINOUT=$(DLLNAME).def $(DLLNAME).lib $(DLLNAME).exp $(DLLNAME).dll
SLIB=darknet.dll
endif
